...
#         if self.get_parameter('trigger_turn_right').value:
#             param_reset = rclpy.parameter.Parameter('trigger_turn_right', rclpy.parameter.Parameter.Type.BOOL, False)
#             self.set_parameters([param_reset])
...
#             twist = Twist()
#             twist.linear.y = -0.2
#             self.get_logger().info("주차 action 실행")
#             self.mecanum_pub.publish(twist)
#             time.sleep(0.38/0.2)
...
#         self.mecanum_pub.publish(Twist())
#         self.is_turning_right = False 
#         self.get_logger().info("우회전 및 탈출 완료, 라인 트레이싱 복귀")
...
LANE_X_LOST_SAFE_LINEAR = 0.0       # hold 시간 지나도 못 찾으면 정지. 필요시 0.05로 아주 느린 직진
...
# 우회전 FSM
ARROW_SIGNAL_WAIT = 0.4           # 황색 점멸/정지 대기. 너무 길면 출발이 늦어짐
...
CROSSWALK_RETRIGGER_COOLDOWN = 1.2
...
class SelfDrivingNode(Node):
    ...
    def button_callback(self, msg):
        # 버튼이 눌리면 start를 True로 복구할 수 있게만 둔다.
    ...
    def park_action(self):
        ...
        self.transition(DriveState.DONE)
    ...
    def handle_line_follow(self, image, result_image):
        ...
        # ---------------------------------------------------------------------
        # if self.right_ready:
        if self.right_ready and not self.right_done:
            self.get_logger().info(f'[RIGHT_TRIGGER] enter ARROW_SIGNAL lane_x={lane_x} cw_y={self.crosswalk_distance} candidate={self.right_candidate}')
            self.right_ready = False
        ...
        if 70 < self.crosswalk_distance and not self.start_slow_down:
            ...
            if self.count_crosswalk == 3:
                ...
                self.get_logger().info(f'[CROSSWALK_SLOW] y={self.crosswalk_distance}')
        else:
            self.count_crosswalk = 0
        ...
        if self.start_slow_down:
            if self.traffic_signs_status is not None:
                ...
                # 최신 YOLO 메시지에서 검증된 큰 red/green만 여기까지 들어온다.
                # 예전 조건(area < 1000)은 작은 오검출 red(area=484)를 정지로 처리했다.
                if traffic_name == 'red':
                    self.mecanum_pub.publish(Twist())
                    self.stop = True
        ...
        if lane_x >= 0 and not self.stop:
            ...
            if now_lost - self.last_lane_lost_log_time > LANE_LOST_LOG_INTERVAL:
                ...
                if self.stop:
                    self.get_logger().warn(f'[STOP_HOLD] lane_x={lane_x} stop={self.stop} state={self.drive_state} cmd_linear={twist.linear.x:.2f} cw_y={self.crosswalk_distance} traffic={getattr(self.traffic_signs_status, "class_name", None)} right_ready={self.right_ready} right_done={self.right_done}')
                else:
                    self.get_logger().warn(f'[LANE_LOST] lane_x={lane_x} stop={self.stop} state={self.drive_state} cmd_linear={twist.linear.x:.2f} right_ready={self.right_ready} right_done={self.right_done}')
        ...
        if now - self.last_debug_log_time > DEBUG_LOG_INTERVAL:
            ...
            self.get_logger().info(
                ...
                f'lane_x={lane_x} linear={twist.linear.x:.2f} '
                f'angular={twist.angular.z:.2f} '
                f'cw_y={self.crosswalk_distance} right_done={self.right_done} '
    ...
    def handle_intersection(self, image, result_image):
        ...
        if self.arrow_direction == 'right':
            ...
            elif elapsed < (RIGHT_TURN_DURATION + 3.0): 
                # 각속도(angular_z)를 완전히 0.0으로 묶어 좌회전 노이즈 유입을 물리적으로 차단합니다.
                self.move(linear_x=self.normal_speed, angular_z=0.0)
