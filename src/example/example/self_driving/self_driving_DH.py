...
PARK_SIDE_TIME    = 1.5     # 주차 수직이동 지속 시간 (초)  ★
PARK_STOP_Y       = 350     # park 표지판 중심 Y 임계값 (px)  ★
PARK_TIMEOUT      = 5.0     # 주차 직진 최대 시간 (초, 안전장치)

# 감지 임계값
CROSSWALK_NEAR_Y  = 300     # crosswalk Y픽셀 임계
CROSSWALK_COUNT   = 2       # 고속 주행이므로 2프레임
ARROW_COUNT       = 2       # 고속 주행이므로 2프레임
PARK_COUNT        = 6       # ★ 오감지 방지: 6프레임 연속 확인
TRAFFIC_AREA_MIN  = 400     # 신호등 최소 감지 면적 (px²)
PARK_AREA_MIN     = 400     # park 최소 감지 면적 (px²)
ARROW_AREA_MIN    = 400     # 화살표 최소 감지 면적 (px²)
...
class SelfDrivingNode(Node):
    ...
    def __init__(self, name):
        ...
        self.test_mode    = self.declare_parameter('test_mode', 'false').value.lower() == 'true'
        ...
        self.create_subscription(
            Bool, "/ros_robot_controller/button", self._button_cb, 10)
    ...
    def _main_sequence(self):
        ...
        if self.test_mode:
            ...
            for i in range(3, 0, -1):
                self.get_logger().info(f"\033[1;33m[시험모드] {i}초 후 자동 출발\033[0m")
                time.sleep(1.0)
            self.get_logger().info("\033[1;32m[시험모드] 출발!\033[0m")
            self._transition(State.LINE_FOLLOW)
        else:
            # 대회 모드: 버튼 대기 (규칙 3)
            self.get_logger().info("\033[1;33mFSM v3 Ready: Button1을 눌러 출발하세요\033[0m")
    ...
    def _button_cb(self, msg: Bool):
        if msg.data and self._fsm_state == State.WAIT_START:
            self.get_logger().info("\033[1;32m[버튼] 출발 신호\033[0m")
            self._transition(State.LINE_FOLLOW)
    ...
    def _transition(self, new_state: str):
        ...
        if new_state == State.LINE_FOLLOW:
            self._led("green_on")
        elif new_state in (State.CROSSWALK_STOP,
                           State.TRAFFIC_LIGHT,
                           State.WAIT_START):
            self._led("red_on")
    ...
    def _state_crosswalk_stop(self, image, result_image):
        ...
        if elapsed < CROSSWALK_WAIT:
            ...
            self._transition(State.LINE_FOLLOW)
            threading.Thread(
                target=self._reset_crosswalk_flag, daemon=True).start()
        return result_image
    ...
    def _state_intersection(self, image, result_image):
        ...
        if self._arrow_direction == "go":
            ...
            if elapsed < TURN_TIME:
                # ✅ 수정 1 반영: twist.angular.z 올바르게 사용
    ...
    def _state_parking(self, image, result_image):
        ...
        # 2단계: 우측 수직이동
            ...
            # ✅ 수정 3: linear_y = +PARK_SIDE_SPEED (진행방향 기준 우측)
            # 중앙 도로에서 좌(←)로 진행 → 우측 = 로봇 기준 +y 방향
            # 실제 이동이 반대 방향이면 부호를 음수로 변경
    ...
    def _do_line_follow(self, lane_x: float, lane_angle: float, speed: float):
        ...
        twist.angular.z = common.set_range(
            self.pid.output + (lane_angle * 0.005), -0.15, 0.15)
        self.cmd_vel_pub.publish(twist)
    ...
    def _draw_objects(self, image):
        for obj in self.objects_info:
            ...
            plot_one_box(obj.box, image, color=color,
