...
LANE_X_LOST_SAFE_LINEAR = 0.05       # hold 시간 지나도 못 찾으면 정지. 필요시 0.05로 아주 느린 직진
...
class SelfDrivingNode(Node):
    ...
    def get_object_callback(self, msg):
        ...
        for obj in self.objects_info:
            ...
            if class_name == 'crosswalk':
                ...
                # park가 YOLO에 실제로 잡히는지 무조건 확인하는 로그
                self.get_logger().info(
                    f'[PARK_DET] class={class_name} '
    ...
    def park_action(self):
        ...
        self.mecanum_pub.publish(twist)
        time.sleep(1.0)
        self.get_logger().info('[PARK_START]')
    ...
    def handle_line_follow(self, image, result_image):
        ...
        if self.stop:
            if self.drive_state == DriveState.PARKING:
                # 주차 상태라면 메인 루프는 아무 모터 명령도 주지 않고 즉시 리턴
                return result_image
    ...
    def handle_intersection(self, image, result_image):
        ...
        if self.arrow_direction == 'right':
            ...
            elif elapsed < (RIGHT_TURN_DURATION + 1.0): 
