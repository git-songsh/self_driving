...
CROSSWALK_CONFIRM_COUNT = 1  # box+pair 조건 자체가 강하므로 1프레임 트리거
CROSSWALK_RETRIGGER_COOLDOWN = 7.5

# 횡단보도 박스 기반 정지 조건
# 같은 방향의 횡단보도 2개가 잡히면 pair로 보고, 가까운 쪽 박스가 이 범위에 들어오면 정지한다.
# 로그 기준: 2만~4.5만 부근이 정지선 앞, 6만 이상은 이미 너무 가까운 경우가 많다.
CROSSWALK_BOX_MIN_AREA = 16000
CROSSWALK_BOX_MAX_AREA = 52000
CROSSWALK_PAIR_MAX_CX_DIFF = 170
CROSSWALK_PAIR_MIN_Y_GAP = 25
CROSSWALK_CLOSE_AREA_RATIO = 1.8
CROSSWALK_PAIR_HOLD_SEC = 1.2
...
class SelfDrivingNode(Node):
    ...
    def param_init(self):
        ...
        # 횡단보도 2개 pair + 가까운 박스 면적 기반 정지 판단
        self.crosswalk_candidates = []
        self.crosswalk_box_ready = False
        self.crosswalk_box_target = None
        self.crosswalk_pair_seen_time = 0.0
        self.crosswalk_pair_close_candidate = None
        self.crosswalk_pair_far_candidate = None
    ...
    def init_process(self):
        ...
        self.get_logger().info('\033[1;32m[START] hybrid self_driving ready: WAIT_START, press button to LINE_FOLLOW\033[0m')
    ...
    def get_object_callback(self, msg):
        ...
        for obj in self.objects_info:
            ...
            if class_name == 'right':
                ...
                if score > 0.40:
                    ...
                    crosswalk_candidates.append(cw)
                    if y2 > min_distance:
        ...
        # ------------------------------------------------------------------
        # 횡단보도 박스 기반 정지 후보 판단 — final min-fix
        # 핵심:
        # 1) 같은 방향 횡단보도 2개가 한 프레임에 잡히면 바로 pair로 인정한다.
        # 2) 정지 판단값은 먼 횡단보도가 아니라 아래쪽/가까운 횡단보도 close 기준으로 쓴다.
        # 3) pair가 한 번 잡히면 1.2초 동안 유지해서 YOLO 프레임 튐을 흡수한다.
        # 4) close area가 16000~52000이면 정지 후보. 로그상 17794, 21068, 23052,
        #    29309, 34854, 37350이 정지선 전후 후보였고, 6만 이상은 너무 가까운 편이었다.
        # ------------------------------------------------------------------
        self.crosswalk_candidates = crosswalk_candidates
        self.crosswalk_box_ready = False
        self.crosswalk_box_target = None
        ...
        if len(crosswalk_candidates) >= 2:
            sorted_candidates = sorted(crosswalk_candidates, key=lambda c: c['y2'], reverse=True)
            for close in sorted_candidates:
                far_candidates = [
                    c for c in crosswalk_candidates
                    if c is not close
                    and close['y2'] > c['y2']
                    and abs(close['cx'] - c['cx']) <= CROSSWALK_PAIR_MAX_CX_DIFF
                    and (close['y2'] - c['y2']) >= CROSSWALK_PAIR_MIN_Y_GAP
                    and close['area'] >= c['area'] * CROSSWALK_CLOSE_AREA_RATIO
                ]
                if far_candidates:
                    # 같은 방향 pair 중 가장 아래쪽 close를 정지 기준으로 사용한다.
                    pair_far = max(far_candidates, key=lambda c: c['y2'])
                    close_for_stop = close
                    self.crosswalk_pair_seen_time = now
                    self.crosswalk_pair_close_candidate = dict(close)
                    self.crosswalk_pair_far_candidate = dict(pair_far)
                    self.get_logger().warn(
                        f'[CW_PAIR_SEEN] close={close_for_stop} far={pair_far} '
                        f'hold={CROSSWALK_PAIR_HOLD_SEC:.2f}s'
                    )
                    break

        pair_recent = (now - self.crosswalk_pair_seen_time) <= CROSSWALK_PAIR_HOLD_SEC

        if close_for_stop is None:
            if crosswalk_candidates:
                # pair가 최근에 확인된 상태에서는 현재 프레임의 가장 아래쪽 후보를 close로 사용한다.
                # 단독 후보만 들어오는 프레임을 구제하기 위한 fallback이다.
                if pair_recent:
                    close_for_stop = max(crosswalk_candidates, key=lambda c: c['y2'])
            elif pair_recent and self.crosswalk_pair_close_candidate is not None:
                close_for_stop = self.crosswalk_pair_close_candidate

        if close_for_stop is not None:
            close_area = int(close_for_stop['area'])
            close_y2 = int(close_for_stop['y2'])
            area_ok = CROSSWALK_BOX_MIN_AREA <= close_area <= CROSSWALK_BOX_MAX_AREA
            y_ok = close_y2 >= 290

            if pair_recent and area_ok and y_ok:
                self.crosswalk_box_ready = True
                self.crosswalk_box_target = {
                    'close': close_for_stop,
                    'pair_close': self.crosswalk_pair_close_candidate,
                    'pair_far': self.crosswalk_pair_far_candidate,
                    'pair_age': round(now - self.crosswalk_pair_seen_time, 3),
                    'n': len(crosswalk_candidates),
                }
                min_distance = close_y2
                crosswalk_status = 'VALID'
                self.get_logger().warn(
                    f'[CW_BOX_READY] close={close_for_stop} target={self.crosswalk_box_target} '
                    f'area_range=({CROSSWALK_BOX_MIN_AREA},{CROSSWALK_BOX_MAX_AREA}) y_ok={y_ok}'
                )
            elif pair_recent:
                # 이전 버전은 왜 안 멈췄는지 추적이 어려웠다. 이번에는 reject 이유를 매번 남긴다.
                self.get_logger().info(
                    f'[CW_BOX_WAIT] close={close_for_stop} pair_recent={pair_recent} '
                    f'area_ok={area_ok} y_ok={y_ok} area_range=({CROSSWALK_BOX_MIN_AREA},{CROSSWALK_BOX_MAX_AREA})'
                )
    ...
    def handle_wait_start(self, image, result_image):
        ...
        if time.time() - self.last_debug_log_time > DEBUG_LOG_INTERVAL:
            ...
            self.get_logger().info('[WAIT_START] waiting for physical button /ros_robot_controller/button')
        return result_image
    ...
    def handle_line_follow(self, image, result_image):
        ...
        # ---------------------------------------------------------------------
        if self.right_ready:
            self.get_logger().info(f'[RIGHT_TRIGGER] enter ARROW_SIGNAL lane_x={lane_x} raw_lane_x={raw_lane_x} filter={lane_filter_status} cw_y={self.crosswalk_distance} candidate={self.right_candidate}')
            self.right_ready = False
        ...
        # ---------------------------------------------------------------------
        # 1-1) 신호등 즉시 처리: red면 정지, green이면 정지 해제
        # ---------------------------------------------------------------------
        if self.traffic_signs_status is not None:
            traffic_name_now = self.traffic_signs_status.class_name
            if traffic_name_now == 'red':
                self.traffic_stop_latched = True
                self.stop = True
                self.stop_robot()
                self.get_logger().info('[TRAFFIC_RED_STOP] red detected -> latch stop')
                return result_image
            elif traffic_name_now == 'green':
                if self.traffic_stop_latched or self.stop:
                    self.get_logger().info('[TRAFFIC_GREEN_GO] green detected -> release red latch')
                self.traffic_stop_latched = False
                self.stop = False
        ...
        if self.traffic_stop_latched:
            self.stop_robot()
            self.stop = True
            self.get_logger().info('[TRAFFIC_RED_HOLD] waiting for green')
            return result_image
        ...
        if crosswalk_can_trigger and self.crosswalk_cw_status == "VALID":
            ...
            if self.count_crosswalk >= CROSSWALK_CONFIRM_COUNT:
                ...
                self.get_logger().warn(
                    f'[CROSSWALK_SLOW] box_stop=True y={self.crosswalk_distance} '
                    f'target={self.crosswalk_box_target} confirm={CROSSWALK_CONFIRM_COUNT}'
                )
        else:
        ...
        if self.start_slow_down:
            ...
            if self.traffic_signs_status is not None:
                ...
                if traffic_name == 'red':
                    ...
                    if self.stop:
                        self.get_logger().info(f'[TRAFFIC_GREEN_RELEASE] area={area} cw_y={self.crosswalk_distance}')
                    self.stop = False
                    self.get_logger().info(f'[TRAFFIC_GREEN_GO] area={area} cw_y={self.crosswalk_distance}')
            else:
            ...
            if not self.stop:
                if lane_x >= 0:
                    ...
                    if self.machine_type != 'MentorPi_Acker':
                        current_angular = common.set_range(self.pid.output, PID_MIN, PID_MAX)
                    else:
                        current_angular = (self.normal_speed * math.tan(common.set_range(self.pid.output, PID_MIN, PID_MAX))) / 0.145
                else:
                    current_angular = 0.0
                ...
                if elapsed < 1.4:
                    ...
                    self.get_logger().info(f'[CW_STAGE 2] 정지선 앞 완전 정지! elapsed={elapsed:.2f}s')
                    return result_image
                else:
                    if traffic_name == 'red':
                        self.move(0.0, 0.0, 0.0)
                    ...
                    self.get_logger().info('[CW_STAGE 3] 미션 완료 -> 정상 속도 회복 및 6초 락 가동')
        ...
        if lane_x >= 0 and not self.stop:
            ...
            if now_lost - self.last_lane_lost_log_time > LANE_LOST_LOG_INTERVAL:
                ...
                if self.stop:
                    self.get_logger().warn(f'[STOP_HOLD] raw_lane_x={raw_lane_x} lane_x={lane_x} filter={lane_filter_status} stop={self.stop} state={self.drive_state} safe_linear={LANE_X_LOST_SAFE_LINEAR:.2f} cw_y={self.crosswalk_distance} traffic={getattr(self.traffic_signs_status, "class_name", None)} right_ready={self.right_ready} right_done={self.right_done}')
                else:
                    self.get_logger().warn(f'[LANE_LOST] raw_lane_x={raw_lane_x} lane_x={lane_x} filter={lane_filter_status} stop={self.stop} state={self.drive_state} safe_linear={LANE_X_LOST_SAFE_LINEAR:.2f} right_ready={self.right_ready} right_done={self.right_done}')
        ...
        if now - self.last_debug_log_time > DEBUG_LOG_INTERVAL:
            ...
            self.get_logger().info(
                ...
                f'cw_y={self.crosswalk_distance} cw_box_ready={self.crosswalk_box_ready} right_done={self.right_done} '
    ...
    def handle_arrow_signal(self, image, result_image): # 우회전 진입 전
        ...
        if elapsed < ARROW_SIGNAL_WAIT:
            ...
            if time.time() - self.last_debug_log_time > DEBUG_LOG_INTERVAL:
                self.last_debug_log_time = time.time()
                self.get_logger().info(f'[ARROW_SIGNAL] waiting elapsed={elapsed:.2f}/{ARROW_SIGNAL_WAIT:.2f} direction={self.arrow_direction}')
        else:
            self.get_logger().info(f'[ARROW_SIGNAL] done elapsed={elapsed:.2f} -> INTERSECTION direction={self.arrow_direction}')
            # self.led('yellow_off')
    ...
    def handle_intersection(self, image, result_image): # 우회전 진입
        ...
        if self.arrow_direction == 'right':
            ...
            if elapsed < RIGHT_TURN_DURATION:
                ...
                self.get_logger().info(f'[RIGHT_TURN] done elapsed={elapsed:.2f} lane_x={lane_x} -> LINE_FOLLOW. . . .')
                # self.transition(DriveState.PARKING)
                self.transition(DriveState.LINE_FOLLOW)
