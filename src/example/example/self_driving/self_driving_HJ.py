...
CROSSWALK_CONFIRM_COUNT = 1  # 박스+pair 조건 자체가 강하므로 3프레임 확인은 너무 늦다
CROSSWALK_RETRIGGER_COOLDOWN = 7.5

# 횡단보도 박스 기반 정지 조건
# 같은 방향 횡단보도가 2개 이상 잡혔을 때, 가까운 쪽 박스 면적이 이 범위에 들어오면 정지 후보로 본다.
# 너무 작으면 멀리 있는 것이고, 너무 크면 이미 정지선을 지나친 뒤일 가능성이 크다.
# 로그 기반 보정:
# - 18,000 이하는 아직 이른 후보가 섞임
# - 20,000 이상부터 실제 접근 구간
# - 85,000 이상은 횡단보도에 너무 붙은 상태라 제외
# - pair 조건은 구간 진입 확인용, area 조건은 정지 위치 확인용
CROSSWALK_BOX_MIN_AREA = 20000
CROSSWALK_BOX_MAX_AREA = 85000
CROSSWALK_PAIR_MAX_CX_DIFF = 130
CROSSWALK_PAIR_MIN_Y_GAP = 25
CROSSWALK_PAIR_HOLD_SEC = 0.70
CROSSWALK_CLOSE_AREA_RATIO = 2.0
...
class SelfDrivingNode(Node):
    ...
    def get_object_callback(self, msg):
        ...
        # ------------------------------------------------------------------
        # 횡단보도 박스 기반 정지 후보 판단 — minfix2
        # 문제: YOLO가 같은 프레임에 횡단보도 2개를 항상 주지 않아 cw_box_ready가 계속 False였다.
        # 수정: pair는 '구간 진입 확인용'으로 0.7초 hold하고, 그 사이 가까운 박스 면적으로 정지한다.
        # ------------------------------------------------------------------
        self.crosswalk_candidates = crosswalk_candidates
        self.crosswalk_box_ready = False
        self.crosswalk_box_target = None

        # 1) 현재 프레임에서 같은 방향 pair가 보이면 pair 상태를 갱신한다.
        if len(crosswalk_candidates) >= 2:
            # 가까운 후보는 보통 면적이 가장 큰 박스다.
            close = max(crosswalk_candidates, key=lambda c: c['area'])
            far_candidates = [
                c for c in crosswalk_candidates
                if c is not close
                and abs(c['cx'] - close['cx']) <= CROSSWALK_PAIR_MAX_CX_DIFF
                and abs(close['y2'] - c['y2']) >= CROSSWALK_PAIR_MIN_Y_GAP
                and close['area'] >= c['area'] * CROSSWALK_CLOSE_AREA_RATIO
            ]

            if far_candidates:
                # 같은 방향 후보 중 y 차이가 가장 큰 것을 far로 본다.
                far = max(far_candidates, key=lambda c: abs(close['y2'] - c['y2']))
                self.crosswalk_pair_seen_time = now
                self.crosswalk_pair_close_candidate = dict(close)
                self.crosswalk_pair_far_candidate = dict(far)
                self.get_logger().info(
                    f'[CW_PAIR_SEEN] close={close} far={far} '
                    f'hold={CROSSWALK_PAIR_HOLD_SEC:.2f}s'
                )

        # 2) pair가 최근에 보였으면, 현재 가장 큰 crosswalk 박스로 정지 위치를 판단한다.
        pair_recent = (now - self.crosswalk_pair_seen_time) <= CROSSWALK_PAIR_HOLD_SEC
        close_for_stop = None
        if crosswalk_candidates:
            close_for_stop = max(crosswalk_candidates, key=lambda c: c['area'])
        elif pair_recent and self.crosswalk_pair_close_candidate is not None:
            close_for_stop = self.crosswalk_pair_close_candidate

        if pair_recent and close_for_stop is not None:
            area_ok = CROSSWALK_BOX_MIN_AREA <= close_for_stop['area'] <= CROSSWALK_BOX_MAX_AREA
            ...
            if area_ok:
                ...
                # 정지 판단에는 먼 횡단보도가 아니라 가까운 횡단보도 y2를 쓴다.
                min_distance = int(close_for_stop['y2'])
                crosswalk_status = "VALID"
                self.get_logger().info(
                    f'[CW_BOX_READY] close={close_for_stop} pair_age={now - self.crosswalk_pair_seen_time:.2f} '
                    f'area_range=({CROSSWALK_BOX_MIN_AREA},{CROSSWALK_BOX_MAX_AREA})'
                )
            else:
                self.get_logger().info(
                    f'[CW_BOX_WAIT] close={close_for_stop} pair_age={now - self.crosswalk_pair_seen_time:.2f} '
                    f'area_range=({CROSSWALK_BOX_MIN_AREA},{CROSSWALK_BOX_MAX_AREA})'
                )
    ...
    def handle_line_follow(self, image, result_image):
        ...
        # ---------------------------------------------------------------------
        # 1-1) 신호등 우선 처리
        # red가 현재 프레임에서 유효하게 보이면 즉시 정지하고, green이면 정지 상태를 해제한다.
        # traffic_signs_status는 get_object_callback()에서 현재 프레임 기준으로 stale 값을 제거한다.
        # ---------------------------------------------------------------------
        if self.traffic_signs_status is not None:
            ...
            if traffic_name_now == 'red':
                self.stop_robot()
                self.stop = True
                self.get_logger().info('[TRAFFIC_RED_STOP] red detected -> stop')
                return result_image
            elif traffic_name_now == 'green':
                if self.stop:
                    self.get_logger().info('[TRAFFIC_GREEN_GO] green detected -> release stop')
                self.stop = False
            ...
            if not self.stop:
                ...
                if elapsed < 1.4:
                    ...
                    if traffic_name == 'red':
                        ...
                        # 초록불을 확인했을 때 로그 출력 후 탈출 시퀀스로 이동
                        self.get_logger().info('GREEN 신호 확인 -> 정상 복귀 프로세스 진행')
                    else:
                        # 신호등이 안 잡히는 상황(None) 등에서도 안전하게 출발
                        self.get_logger().info('신호등 없음 -> 정상 복귀 프로세스 진행')
        ...
        if now - self.last_debug_log_time > DEBUG_LOG_INTERVAL:
            ...
            self.get_logger().info(
                ...
                f'cw_y={self.crosswalk_distance} cw_box_ready={self.crosswalk_box_ready} '
                f'right_done={self.right_done} '
